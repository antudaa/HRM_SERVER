...
LeaveTypeSchema.statics.getApplicableForEmployee = async function (
  ...
  // Extend with employee-policy matching later
...
LeaveTypeSchema.statics.computeEntitlementForEmployee = async function (
  ...
  // Placeholder — link to aggregated balance later if needed
...
LeaveTypeSchema.statics.canApply = async function (
  ...
  if (durationRules?.allowedIncrements?.length) {
  const eps = 1e-9;
  const ok = durationRules.allowedIncrements.some((inc) => {
    const q = params.unitsRequested / inc;
    return Number.isFinite(q) && Math.abs(q - Math.round(q)) < eps;
  });
  if (!ok) return { ok: false, reason: "Invalid increment requested" };
} else if (
  durationRules?.minIncrement &&
  Math.abs((params.unitsRequested / durationRules.minIncrement) - Math.round(params.unitsRequested / durationRules.minIncrement)) > 1e-9
) {
  return { ok: false, reason: "Requested units not aligned to min increment" };
}
