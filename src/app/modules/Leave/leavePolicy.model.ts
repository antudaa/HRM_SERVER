...
const LeavePolicySchema = new Schema<TLeavePolicy, LeavePolicyModel>(
  {
...
LeavePolicySchema.index({ orgId: 1, leaveTypeId: 1, year: 1 }, { unique: true });

LeavePolicySchema.statics.getActiveFor = function (orgId, leaveTypeId, year) {
  return this.findOne({ orgId: orgId ?? { $exists: false }, leaveTypeId, year, active: true });
};

export const LeavePolicy = model<TLeavePolicy, LeavePolicyModel>("LeavePolicy", LeavePolicySchema);
