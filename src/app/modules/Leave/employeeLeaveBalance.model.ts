...
const EmployeeLeaveBalanceSchema = new Schema<TEmployeeLeaveBalance, EmployeeLeaveBalanceModel>(
  {
...
EmployeeLeaveBalanceSchema.statics.upsertForEmployeeYear = async function (
...
EmployeeLeaveBalanceSchema.statics.getForEmployeeYear = function (employeeId: Types.ObjectId, year: number) {
  return this.find({ employeeId, year }).lean();
};

EmployeeLeaveBalanceSchema.statics.recomputeFromLedger = async function (
  employeeId: Types.ObjectId,
  leaveTypeId: Types.ObjectId,
  year: number
) {
...
EmployeeLeaveBalanceSchema.statics.postLedgerAndRecompute = async function (
  ledger: LedgerInput,
